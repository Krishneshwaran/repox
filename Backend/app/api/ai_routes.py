...
async def summarize_repo(
    ...
    summary = ai_service.summarize(payload.repo_name, payload.scan_id, payload.scan_result, payload.clone_url)
...
async def architecture_overview(
    ...
    report = ai_service.architecture(payload.repo_name, payload.scan_id, payload.scan_result, payload.clone_url)
...
async def readme_analysis(
    ...
    report, scores = ai_service.readme_analysis(payload.repo_name, payload.clone_url, payload.scan_result)
...
async def repository_insights(
    ...
    report = ai_service.insights(payload.repo_name, payload.scan_id, payload.scan_result, payload.clone_url)
...
async def ask_repo(
    ...
    answer = ai_service.ask(payload.repo_name, payload.question, payload.scan_id, payload.scan_result, payload.clone_url)
